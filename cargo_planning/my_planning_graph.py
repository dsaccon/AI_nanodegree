...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            pgn_a = PgNode_a(action)
            if pgn_a.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(pgn_a)
                for pgn_s in self.s_levels[level]:
                    pgn_a.parents.add(pgn_s)
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            this_levelcost = False
            while level < len(self.s_levels) and not this_levelcost:
                for state in self.s_levels[level]:
                    if goal == state.symbol and state.is_pos == True:
                        level_cost.append(level)
                        this_levelcost = True
                        break
                level += 1

        return sum(level_cost)